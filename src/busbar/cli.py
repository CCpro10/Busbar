...
from .backends import DEFAULT_MODEL, DEFAULT_REVISION, load_backend
...
def parser() -> argparse.ArgumentParser:
    ...
    for name in ("run", "serve", "benchmark"):
        ...
        command.add_argument("--backend", choices=("mlx", "hf"), default="mlx")
        command.add_argument("--model", default=DEFAULT_MODEL)
        command.add_argument("--revision", default=DEFAULT_REVISION)
        command.add_argument("--dtype", choices=("float16", "float32"))
        ...
        if name == "run":
            ...
            command.add_argument("--projection", choices=("selected", "full"), default="selected")
        elif name == "serve":
...
def main():
    ...
    try:
        if args.command == "benchmark" and args.output.exists():
