...
输出目录只能新建。报告和 manifest 先完整写入临时文件，再原子发布且不覆盖已有文件；manifest 最后出现，表示版本已完成。写入中断或训练失败会留下不完整目录，加载器会拒绝；排查后换新目录重试，旧版本仍可用。加载检查固定文件名、文件哈希、头结构、每个张量的名称/形状/FP32/有限值、底座 revision 和 dtype，并直接解析已验证的权重及报告字节，避免校验后文件替换导致版本不一致。运行时 snapshot 身份包含头版本和编码器版本，两个头不能共用旧快照。

数据只读取一次，训练记录的 SHA256、标签和划分都来自同一份捕获内容；训练结束再次检查源文件是否发生变化，变化时不发布选择头。0.4.1 保持 0.4 的版本目录格式，旧头无需重新训练即可加载。
